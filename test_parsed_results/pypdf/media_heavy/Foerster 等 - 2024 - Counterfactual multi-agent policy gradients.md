...
1University of Oxford, United Kingdom †Equal contribution
Abstract
...
called counterfactual multi-agent (COMA) policy gradients,
in order to address these issues. COMA takes anactor-critic
...
t =Pn
l=1 γl−1rt+l +
γnf c(·t+n, θc). In particular, the critic parametersθc are up-
dated by minibatch gradient descent to minimise the follow-
ing loss:
Lt(θc) = (y(λ) − f c(·t, θc))2, (2)
where y(λ) = (1 − λ)P∞
n=1 λn−1G(n)
t , and the n-step
returns G(n)
t are calculated with bootstrapped values esti-
...
user-specified default actionca that can be difficult to choose
...
an advantage function that compares theQ-value for the cur-
rent action ua to a counterfactual baseline that marginalises
out ua, while keeping the other agents’ actionsu−a fixed:
Aa(s, τ , u) = Q(s, τ , u)−
X
u′a
...
map heur. IAC- V IAC- Q cnt- V cnt- QV COMA heur. DQN GMEZOmean best
...
values in parentheses denote the 95% confidence interval, for example87(3) = 87 ±3. Also shown, maximum win percentages
...
learning. Journal of Artificial Intelligence Research77:295–
354.
Lyu, X.; Baisero, A.; Xiao, Y .; Daley, B.; and Amato, C.
...
air traffic flow management. InProceedings of the 6th inter-
...
gm = Eπm [∇θ log πm(um|sm)Q(sm, um)] , (18)
...
update for each timestep. Both the actor and the critic networks are trained using RMS-prop with learning rate0.0005 and alpha
0.99, without weight decay. We set gamma to0.99 for all maps.
