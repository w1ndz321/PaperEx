...
˜P, we define an automated extraction procedure that identifies the core
Based on the de-identified paper pool
...
different sections to collect complementary methodological signals. Let x 𝑝 = (intro 𝑝, method 𝑝, exp 𝑝) denote the
...
We define a method unit 𝑢 ∈ U𝑝 as a self-contained description of how a research problem is formulated or solved,
...
z 𝑝 = 𝑔(U𝑝),
where U𝑝 denotes the set of extracted method units for paper 𝑝 and 𝑔(·) is an embedding function that maps a set of
...
unit set U𝑝, we add directed edges between pairs of method units (𝑢𝑖, 𝑢 𝑗 ) ∈ U𝑝 × U𝑝 to indicate that they are jointly
...
structured retrieval problem over the knowledge graph G. Let 𝑞 denote the input research idea, and let C = {𝐶1, . . . , 𝐶𝑀 }
...
𝑣∈ V
where V = {idea, domain, paper} indexes the retrieval views, 𝑠𝑣 (·) denotes a view-specific scoring function, and 𝜆𝑣 are
...
denote a semantic similarity function between 𝑞 and an idea 𝑖 ∈ I. The idea-level score of a research pattern 𝐶𝑚 is
...
Domain-level retrieval. At the domain level, the system interprets the input idea 𝑞 in terms of its underlying research
domains and methodological themes. Let D denote the set of research domains, and let simdomain(𝑞, 𝑑) measure the
relevance between 𝑞 and domain 𝑑 ∈ D. The domain-level score of pattern 𝐶𝑚 is computed as
...
where simpaper (𝑞, 𝑝) measures semantic similarity between 𝑞 and paper 𝑝, and 𝛼( 𝑝) denotes a quality-related weight
...
tion area.
In contrast, paper-level nodes are typically
...
sification fails to capture.
Intent understanding is
