...
ing via Group Relative Policy Optimization rorscantriggercompliancerisks,userharm,and
(GRPO)withmulti-dimensionalrewardscover-
...
onlineA/Btestdemonstratesa28.6%increase evidence-groundedresponses.
...
etal.,2017a;Rafailovetal.,2023)canaligngener- 2.2 Graph-awareRetrieval
...
dlingcomplexmulti-hopdependenciesanddomain-
generationasisolatedstagesleavesacoordination
...
complementedbyaparallelretrievalarchitecture
optimizesretrievalandevidence-groundedgener-
...
nology alignment (Edge et al., 2024); and (2) tational overhead. To balance effectiveness and
Evidence-constrainedReinforcementLearning efficiency,wemaintainahigh-citationknowledge
(RL), which aligns the generator with retrieved baseK h ⊂ K throughtraffic-drivenfeedback. We
evidenceusingmulti-dimensionalrewardsthaten- accumulate recall frequency for each knowledge
couragefaithfulnesswhileenforcingstyle,safety, chunk from production query logs as a “citation
andURLvalidity.
heat”indicator,andperiodicallyselectthetop-N%
most frequently cited items to form K . This cu-
Ourcontributionsareasfollows: h
ratedsubsetservesasthesubgraphforGraphRAG,
• We propose a co-adaptation framework that reducingtraversalcomplexitywhilepreservingef-
...
alignment between retrieved domain knowl- GraphRAG Architecture. We construct a
edgeandgeneratedresponses. knowledge graph G = (V,E) over K h via entity
extractionandrelationidentification,withcommu-
• Wedesignamulti-dimensionalRLobjective nity detection partitioning the graph into hierar-
...
inURLhallucination. Parallel Retrieval Architecture. To mitigate
...
asynchronous graph traversal over K for multi-
h
...
dezilamroN
Metric BaseRAG Ours ∆
LikeRate(%) 0.21 0.27 +28.6%
DislikeRate(%) 0.26 0.18 −30.8%
Avg.Conv.Turns 1.54 1.81 +17.5%
Table2: OnlineA/Btestingat50%traffic.
...
Style Safety swerable 44.60%→53.40%, Counterfactual
Faithfulness Overall 57.90%→64.40%(outperformingDeepSeek-V3.2
Link
...
missingormisleading. OnInconsistent,itreaches
Figure4: Trainingdynamicsofmulti-dimensionalre-
wardcomponentsduringRL. 84.60%,substantiallyaboveQwen3-32B(63.80%)
and closer to DeepSeek-V3.2 (94.80%). These
results indicate improved contextual faithfulness
3.4 RLRewardEffectiveness
withoutdegradinggeneralization.
...
stratingefficientrewarddesign. V3(Liuetal.,2024)baselineviaexpertassessment
...
6
EthicsStatement PatrickLewis,EthanPerez,AleksandraPiktus,Fabio
...
provesystemreliabilityandsafetybyreducingun- volume33,pages9459–9474.
...
scientificanalysisanddonotnecessarilyreflectthe 2024. Deepseek-v3technicalreport. arXivpreprint
...
thedevelopmentofmoresecureandreliabledigital Paranjape,MicheleBevilacqua,FabioPetroni,and
...
pages9004–9017,Singapore.AssociationforCom-
...
02-08.
TianhongGao,JundongShen,JiapengWang,BeiShi,
...
andHaofenWang.2024. Retrieval-augmentedgen- model is secretly a reward model. In Advances in
erationforlargelanguagemodels: Asurvey. arXiv NeuralInformationProcessingSystems.
...
longcontext,andnextgenerationagenticcapabilities. generationmodelsforquestionansweringandreduc-
Technicalreport,accessed2026-02-08. inghallucination. arXivpreprintarXiv:2410.17783.
...
Sheng, Lianmin Zheng, Cody Hao Yu, Joseph E. arXiv:1707.06347.
...
2. Style Compliance, score 0-10: evaluation set from 1,000 knowledge items sam-
...
specific formatting requirements? sothatallmodelsreceiveidenticalinputs. Allmod-
...
100 7-8 good. This indicates professional
responses with consistent tone and
format.
9-10 excellent. This indicates full 80
...
e
?
vere violations. This
Ours DeepSeek-V3
K
.2 imi K2.5 Doubao 1.8 HY 2.0
...
tionscreenshots,andprovidesadditionalguidance
...
I.AccountIDQueryforUncertifiedAdvertisingAc-
1. Logintotheserviceprovidersystem;
counts
...
certifiedaccounts,pleaserefertothedetailedstepsand II.AccountIDQueryforCertifiedAdvertisingAc-
screenshotsinthefollowinglinks: counts
