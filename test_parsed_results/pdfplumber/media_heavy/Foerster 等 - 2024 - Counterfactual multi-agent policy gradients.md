...
need for new reinforcement learning methods that can ef- Kraemer and Banerjee 2016) and has recently been picked
...
actionsfixed.COMAalsousesacriticrepresentationthatal- making it difficult for each agent to deduce its own con-
lowsthecounterfactualbaselinetobecomputedefficientlyin tribution to the team’s success. Sometimes it is possible to
...
criticmethodsinthissetting,andthebestperformingagents
...
Manycomplexreinforcementlearning(RL)problemssuch (Konda and Tsitsiklis 2000) approach, in which the actor,
as the coordination of autonomous vehicles (Cao et al. i.e., thepolicy, istrained byfollowing agradient estimated
...
urally modelled as cooperative multi-agent systems. How- usedduringlearning,whileonlytheactorisneededduring
ever, RL methods designed for single agents typically fare execution.Sincelearningiscentralised,wecanthereforeuse
poorly on such tasks, since the joint action space of the acentralisedcriticthatconditionsonthejointactionandall
...
tionconstraintsduringexecutionmaynecessitatetheuseof
a shaped reward that compares the global reward to the re-
decentralisedpoliciesevenwhenthejointactionspaceisnot
...
is zero. Hence, instead of relying on extra simulations, ap- andthecriticconditiononlocal,per-agent,observationsand
...
Third,COMAusesacriticrepresentationthatallowsthe state, and control of all units, although the architecture of
...
forallagents,allQ-valuesforallagentscanbecomputedin tion,whilePengetal.(2017)useanactor-criticmethodthat
...
cromanagement1,whichhasrecentlyemergedasachalleng-
al.(2017),whoalsouseamulti-agentrepresentationandde-
ingRLbenchmarktaskwithhighstochasticity,alargestate- centralised policies. However, they focus on stabilising ex-
actionspace,anddelayedrewards.Previousworks(Usunier periencereplaywhileusingDQNanddonotmakefulluse
...
meaningfullydecentralisedbenchmarkthatproveschalleng- fullyobservablesetting.InSection6wethereforereportour
ingforscenarioswithevenrelativelyfewagents,wepropose competitiveperformanceagainstthesestate-of-the-artbase-
...
COMA can significantly improve performance over other ingregime.
multi-agentactor-criticmethods,aswellasablatedversions (Lowe et al. 2017) concurrently propose a multi-agent
...
givenaccesstofullstateinformationandmacro-actions. likeourwork,itlearnsaseparatecentralisedcriticforeach
agentandisappliedtocompetitiveenvironmentswithcon-
2 RelatedWork tinuousactionspaces.
Although multi-agent RL has been applied in a variety of Our work builds directly off of the idea of difference
settings(Busoniu,Babuska,andDeSchutter2008;Yangand rewards (Wolpert and Tumer 2002). The relationship of
...
byLeiboetal.(2017)tostudytheemergenceofcollabora-
...
samerewardfunctionr(s,u) : S ×U → Randγ ∈ [0,1)
...
t t
AA11 tt CCrriittiicc AA22 tt 𝜋a t =𝜋(ha t , 𝜀) Aa t
𝜋𝜋((hh11,, 𝜀𝜀)) 𝜋𝜋((hh22,, 𝜀𝜀))
(𝜀) (ua, 𝜋a) COMA
t t
hh11 uu11 uu22 hh22
tt tt ha {Q(ua=1, u-a,..),. .,Q(ua=|U|, u-a,..)}
AAccttoorr 11 AAccttoorr 22 t t t
((hhaa )) GRU (ha)
o, s r ,h tt--11 t
t t t t-1
oo11 uu11 uu22 oo22
tt tt tt tt
EEnnvviirroonnmmeenntt (oa, a, ua ) (u-a, s, oa, a, u ,h)
t t-1 t t t t-1 t
(a) (b) (c)
...
g =∇ logπ(ua|τa)(r+γV(s )−V(s )). (3) (cid:88)
θπ t t t+1 t Aa(s,τ,u)=Q(s,τ,u)− πa(u′a|τa)Q(s,τ,(u−a,u′a)).
...
tionscontributetothatglobalreward.Sincetheotheragents factuals in which only a’s action changes, learned directly
...
because r(s,(u−a,ca)) does not depend on agent a’s ac- line’s expected contribution to the gradient, as with other
...
tionsspacesbyestimatingtheexpectationin(4)withMonte dardsetupforcentralisedStarCraftcontrolhasthreeeffects.
...
TD(1)criticfollowingaCOMApolicygradient
Figure2:Startingpositionwithexamplelocalfieldofview
(cid:34) (cid:35) forthe2d 3zmap.
(cid:88)
...
Proof. SeeAppendixA. validattackcommandsatsuchenemies,whichresultsinno
...
Inthissection,wedescribetheStarCraftproblemtowhich ofbothexplorationandcontrol.
we apply COMA, as well as details of the state features, Underthesedifficultconditions,scenarioswithevenrela-
...
beeasilyemulated.Manysimplermulti-agentsettings,such andthenfocustheirfire,attackingeachenemyinturnuntil
asPredator-Prey(Tan1993)orPacketWorld(Weyns,Helle- it dies. This heuristic achieves a 98% win rate on 5m with
...
sible,thoughcomputationallyexpensive,tocomputediffer- whichenemyandallyunitsarealiveoroutofview.
...
trol of individual units’ positioning and attack commands arewardequaltotheteam’sremainingtotalhealthplus200.
as they fight enemies. This task is naturally represented as Thisdamage-basedrewardsignaliscomparabletothatused
...
servation of the agent whose actions are being evaluated. forbothCOMAandourbaselines.Ourimplementationuses
...
beforefiringagain.Shieldsabsorbdamageuntiltheybreak,after Ablations. We perform ablation experiments to validate
...
V. The method central-V still uses a central state for the bestperformingandmostconsistentmethod.
...
ously and estimates the advantage as Q − V, replacing both given a full field of view and access to attack-move
...
ever,theseresultssuggestthattheimprovedaccuracyofpol-
ofagents,wherecentralisedcriticsaremoredifficulttotrain
...
agenerousgrantfromMicrosoftfortheirAzurecloudcom- Foerster, J.; Nardelli, N.; Farquhar, G.; Torr, P.; Kohli, P.;
...
ments and discussion. We also thank Gabriel Synnaeve, The34thInternationalConferenceonMachineLearning.
...
An earlier version of this paper contained an error in the q-learning for partially observable mdps. arXiv preprint
...
fiedsetting.TheproofalsoreferstotheSuttonetal.(1999) toplayguesswho?andinventingagroundedlanguageasa
...
be irreducible, which does not hold for history-based state rithms. In Advances in neural information processing sys-
representations.ThankstoFransOliehoekandChrisAmato tems,1008–1014.
...
References Multi-agentcooperationandtheemergenceof(natural)lan-
...
telligence,538–545. MorganKaufmannPublishersInc.
Mordatch,I.,andAbbeel,P. 2017. Emergenceofgrounded
...
preprintarXiv:1703.10069. ing for multi-robot systems: A survey. Technical report,
tech.rep.
Proper, S., and Tumer, K. 2012. Modeling difference re-
wards for multiagent learning. In Proceedings of the 11th Ye,D.;Zhang,M.;andYang,Y.2015.Amulti-agentframe-
InternationalConferenceonAutonomousAgentsandMulti- workforpacketroutinginwirelesssensornetworks.sensors
...
Schulman, J.; Moritz, P.; Levine, S.; Jordan, M. I.; and forthirdpartylogisticsine-commerce. ExpertSystemswith
...
tions. NewYork:CambridgeUniversityPress.
Sukhbaatar, S.; Fergus, R.; et al. 2016. Learning multia-
gentcommunicationwithbackpropagation. InAdvancesin
...
=− p(s,τ) π−a(u−a|τ−a) πa(ua|τa)∇ logπa(ua|τa)b(s,τ,u−a) , (10)
θ
s,τ a u−a ua
(cid:18) (cid:19)
(cid:88) (cid:88)(cid:88) (cid:88)
...
• U =U;
m
• P (s′ |s ) = P(s′,τ′|s,τ) = P(s′|s)1 (z′,z), where τ′ is τ extended with z′, z = [O(s′,1),...,O(s′,n)], and
m m m =
1 (z′,z)=1iffz′ =z;and
=
• r ((s,τ),u)=r(s,u).
...
t=t+1
foreachagentado
ha =Actor (cid:0) oa,ha ,ua ,a,u;θ (cid:1)
t t t−1 t−1 i
