...
WANGYANG YING, School of Computing and Augmented Intelligence, Arizona State University,
Tempe, AZ, USA
DONGJIE WANG, Department of Computer Science, University of Kansas, Lawrence, KS, USA
...
a score (e.g., relevance between feature and label) and select top-푘features as the optimal feature
...
from exponentially growing discrete search space (e.g., around 2푁if the feature number is N). Can
...
feature selection, e.g., 푓1푓2, ..., 푓7 →푓1푓2푓4푓6, as a sequential generative learning task to generate an
...
subset [푓1, 푓2, 푓4, 푓7], we convert it to a feature token sequence denoted as [푆푂푆,푡1,푡2,푡4,푡7, 퐸푂푆].
Sequential Training Data. To construct a differential embedding space for feature selection, we
need to collect 푁different feature subset-accuracy pairs from the original feature set as training
data. Then we convert all feature subsets to feature token sequences. These data are denoted by
푅= (t푖, 푣푖)푁
푖=1, where t푖= [푡1,푡2, ...,푡푞] is the feature token sequence of the 푖th feature subset and 푣푖
is corresponding downstream predictive accuracy.
Problem Statement. Formally, given a tabular dataset 퐷= (푋,푦), where 푋is an original feature
set and y is the corresponding target label. We collect the sequential training data 푅by conducting
automatically traditional feature selection algorithms on 퐷and evaluating the performance of
feature subsets with a downstream ML model. Our goal is to (1) embed the knowledge of 푅into a
differentiable continuous space and (2) generate the optimal feature subset. Regarding goal 1, we
learn an encoder 휙, an evaluator 휗, and a decoder 휓via joint optimization to get the feature subset
ACM Transactions on Knowledge Discovery from Data, Vol. 18, No. 9, Article 221. Publication date: October 2024.
221:4
W. Ying et al.
...
퐸∈E M(푋[휓(퐸)],푦),
(1)
where 휓is a decoder to generate a feature token sequence from any embedding of E; 퐸∗is the
optimal feature subset embedding; M is a downstream ML task. 푋[] means we use the mapping
table to convert a feature token sequence to a feature subset. Finally, we apply f∗to 푋to select the
optimal feature subset 푋[t∗].
3
Methodology
3.1
Framework Overview
...
3.2
Feature Subset Embedding Space Construction via Variational Transformer
...
to quickly collect more legal data samples. For instance, give one sample [푡1,푡2,푡3] →0.867. We
can shuffle the order of the sequence to generate more semantically equivalent data samples:
[푡2,푡1,푡3] →0.867, [푡3,푡2,푡1] →0.867. The shuffling augmentation strategy enhances both the
...
푅= (t푖, 푣푖)푁
푖=1, where t푖= [푡1,푡2, ...,푡푞] is a feature token sequence of the 푖th feature subset, 푣푖is
the corresponding predictive accuracy, 푞is the number tokens of the 푖th feature token sequence,
and 푁is the number of training samples. To simplify the notation, we use the notation (t, 푣) to
represent any training sample. We first employ a transformer encoder 휙to learn the embedding of
the feature token sequence, denoted by e = 휙(t). We assume that the learned embeddings e follow
the format of normal distribution. Then, two fully connected layers are implemented to estimate
the mean m and variance 휎of this distribution. After that, we can sample an embedding vector e∗
from the distribution via the reparameterization technique. This process is denoted by
e∗= m + 휀∗푒푥푝(휎),
(2)
where 휀refers to the noised vector sampled from a standard normal distribution. The sampled
...
푡푗, and the previously completed feature token sequence is 푡1...푡푗−1. The probability of the 푗th token
should be
푃휓(푡푗|e∗, [푡1,푡2, ...,푡푗−1]) =
푒푥푝(푧푗)
Í
푞푒푥푝(푧),
(3)
where 푧푗represents the 푗th output of the softmax layer, 휓refers to the decoder. The joint estimated
...
¥푣= 휗(e∗),
(5)
where 휗refers to the evaluator and ¥푣is the predicted accuracy via 휗.
...
L = 훼L푒푣푡+ 훽L푟푒푐+ 훾L푘푙,
(9)
where 훼, 훽, and 훾are hyperparameters.
ACM Transactions on Knowledge Discovery from Data, Vol. 18, No. 9, Article 221. Publication date: October 2024.
Feature Selection as Deep Sequential Generative Learning
221:7
3.3
Gradient-Steered Optimization
After obtaining the feature subset embedding space, we employ a gradient-ascent search method to
find better feature subset embedding. More specifically, we initiate the process by selecting the top-푘
...
accuracy. The gradient utilized in this process is derived from the well-trained evaluator 휗. Taking
the embedding e∗as an illustrative example, the moving calculation process is as follows:
e+ = e∗+ 휂휕휗
휕e∗,
(10)
where 휂is the moving steps and e+ is the better embedding.
3.4
...
to illustrate the generation process. In the 푗-iteration, we assume that the previously generated
feature token sequence is 푡1...푡푗−1 and the waiting to generate token is 푡푗. The estimation probability
for generating 푡푗is to maximize the following likelihood based on the well-trained decoder 휓:
푡푗= arg max(푃휓(푡푗|e+, [푡1, ...,푡푗−1]).
(11)
We will iteratively generate the possible feature tokens until finding the end token (i.e., <EOS>). For
instance, if the generated token sequence is “[푡2,푡6,푡5, < 퐸푂푆>,푡8], we will cut from the <EOS>
token and keep [푡2,푡5,푡6] as the final generation result. Finally, we select the corresponding features
...
procedure.
3.5
Improvements: Reinforced Data Collector for Sequential Training Data
...
Input
:The original dataset 퐷= (푋,푦)
Output:The Optimal Feature Subset 푋[t∗]
1 Collecting training data set 푅= (t푖, 푣푖)푁
푖=1.
2 Initialize the encoder 휙, decoder 휓and evaluator 휃.
3 Feature Subset Embedding Space Construction:
4 for 푖푛푒푝표푐ℎdo
5
for 푖푛푛푢푚푏푒푟표푓푏푎푡푐ℎ푒푠do
6
Encode: e = 휙(t).
7
Estimate: m, 휎.
8
Reparameterization: e∗= m + 휀∗푒푥푝(휎).
9
Decode loss: L푟푒푐= −푙표푔푃휓(t|e∗).
10
Evaluate loss: L푒푣푡= 푀푆퐸(푣,휃(e∗)).
11
KL loss: L푘푙= 푒푥푝(휎) −(1 + 휎) + (m)2.
12
Backward: L = 훼L푒푣푡+ 훽L푟푒푐+ 훾L푘푙
13
end
14 end
15 Gradient-steered Optimization:
16 Select top-푘feature token sequences (t)푘from 푅.
17 Encode and Reparameterization: (e∗)푘= 푟푒푝푎푟푎푚푒푡푒푟푖푧푎푡푖표푛(휙((t)푘)).
18 Update (e∗)푘with 휂steps: (e+)푘= (e∗)푘+ 휂∗
휕휗
휕(e∗)푘.
19 Optimal Feature Subset Generation:
20 Generation: (t+)푘= 휓((e∗)푘).
21 Optimal feature subset: X[t∗] = arg max M(푋[(t+)푘],푦).
...
feature subset-accuracy training data pairs (e.g., 푓1푓4푓6 →0.817) and construct feature subset
embedding space. Data subset B was never seen by our method. After determining the optimal
feature token sequence, such as 푓2푓5푓6퐴, using Data subset A, we directly applied this feature token
sequence to Data subset B, yielding the feature subset {푓2, 푓5, 푓6}퐵. This feature subset was used
...
algorithms: (1) K-BEST [48] selects the top-푘features with the highest importance scores; (2) mRMR
...
forward network, with each layer having a dimension of 200. The values of 훼, 훽, and 훾are 0.8, 0.2,
...
4.3
Study of the Influence of Variational Transformer for Continuous Space
Construction
...
4.4
Study of the Effectiveness of RL-Based Data Collector
...
4.5
Study of the Impact of Data Augmentation
...
4.6
Study of the Training Data Consistent Quality Collected by RL-Based Data Collector
and Shuffling
...
subset.
4.7
Study of the Time and Space Complexity
...
feature number increases from 21 (SVMGuide3) to 10,936 (AP_omentum) (520-fold increase), there
...
ML models.
4.9
Case Study: VTFS Exhibits Noise Resistance and Quality Feature Attention
...
and the real ones. Here, we take openml_607 and openml_618 datasets as examples. Both of them
have 5 real features and 45 fake features. We employ MARLFS [25] to serve as a comparative model
alongside VTFS. Figure 12 shows the comparison results. Regarding the openml_607 dataset, we
can find that VTFS selects 7 features, of which 4 are real and 3 are fake. In contrast, MARLFS selects
27 features, with only 4 being real and the remaining 23 being fake. For the openml_618 dataset,
...
273–324.
[20]
Riccardo Leardi. 1996. 3 - Genetic Algorithms in Feature Selection. In Genetic Algorithms in Molecular Modeling,
...
[47]
Jihoon Yang and Vasant Honavar. 1998. Feature Subset Selection Using a Genetic Algorithm. In Feature Extraction,
...
[48] Yiming Yang and Jan O. Pedersen. 1997. A comparative study on feature selection in text categorization. In International
