...
1. **title**：论文的完整标题。优先保留预提取标题（它通常来自 PDF 第一行的 # 标题），除非它明显是期刊名、版权声明或作者列表等非标题内容，否则不要修改
...
3. **关联性**：relation 的 head/tail 必须对应已抽取的 concept term
4. **覆盖度**：原文中出现的概念全部提取，有明确语义关系的建立 relation。输出 token 上限8000，concept 和 relation 均衡分配，不要互相挤占"""
